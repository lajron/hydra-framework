...
| `export-adapters [--check] [--dry-run] [--profile <name>]` | Generate provider skills and subagents; `--profile` previews a capability profile's selection without changing generated bytes |
| `profile list\|show` | Inspect capability profiles and their tag-based skill selection; inspection-only until reconciliation is separately admitted |
